...
# Número de registros que se quieren generar
num_registros = 20000  # Ajusta según lo necesites
...
ubicaciones = pd.DataFrame({
    ...
    'pais': [fake.country() for _ in range(10)],
...
tipo_incidencia = tipo_incidencia * (num_registros // len(tipo_incidencia))  # Asegura que haya suficientes elementos

soporte = pd.DataFrame({
    'id_soporte': range(1, num_registros + 1),
    'tipo_incidencia': tipo_incidencia[:num_registros],  # Lista con longitud asegurada
    'descripcion': [fake.sentence() for _ in range(num_registros)],  # Descripción generada por Faker
    'tiempo_resolucion': [round(random.uniform(1, 48), 2) for _ in range(num_registros)]  # Resolución con longitud adecuada
})
